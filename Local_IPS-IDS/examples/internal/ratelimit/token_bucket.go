...
func (mrl *MultiLevelRateLimiter) AllowEndpoint(ctx context.Context, endpoint string) (bool, error) {
	...
	if !exists {
		mrl.mu.Lock()
		limiter = NewTokenBucket(10, 1.0/6.0, mrl.logger) // 10 requests per minute for sensitive endpoints
...
func (mrl *MultiLevelRateLimiter) AllowUser(ctx context.Context, userID string) (bool, error) {
	...
	if !exists {
		mrl.mu.Lock()
		limiter = NewTokenBucket(1000, 1000.0/3600.0, mrl.logger) // 1000 requests per hour
