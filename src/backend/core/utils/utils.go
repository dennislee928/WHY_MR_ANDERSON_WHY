...
func (su *SystemUtils) ExecuteCommand(command string, args ...string) (string, error) {
	...
	output, err := validator.ExecuteCommand(command, args...)
	if err != nil {
...
// ExecuteCommandUnsafe 執行系統命令（不安全，僅用於向後兼容）
// 已棄用: 請使用 ExecuteCommand
func (su *SystemUtils) ExecuteCommandUnsafe(command string, args ...string) (string, error) {
